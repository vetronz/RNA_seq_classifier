...
View(status[idx,][, c('my_category_2', 'most_general', 'Diagnosis')])
# getwd()
...
p<-ggplot(k2.df, aes(cluster, fill=system)) +
...
p<-ggplot(k2.df[bct,], aes(cluster, fill=category)) +
  labs(title = "Barplot of Microbiology Results by Cluster", x = "Diagnosis", y = "Counts")+
...
p<-plot_ly(status[idx,], x = k4$cluster, y = ~status[idx,]$WBC, type = "box",
  color = ~k4$cluster, colors = c(k4.pal)) %>%
  layout(boxmode = "group",
         title = 'Box and Whisker Plot of WBC Count by Diagnostic Group, Split by Gender', 
         xaxis = list(title = 'Diagnosis'),
         yaxis = list(title = 'WBC Count'))
...
p<-plot_ly(status[idx,], x = k4$cluster, y = ~as.numeric(as.character(status[idx,]$array.contemporary.CRP)),
           type = "box", color = ~k4$cluster, colors = c(k4.pal)) %>%
  layout(boxmode = "group",
         title = 'Box and Whisker Plot of WBC Count by Diagnostic Group, Split by Gender', 
         xaxis = list(title = 'Diagnosis'),
         yaxis = list(title = 'CRP Count'))
...
# K4 analysis
k2.df$cluster.4 <- k4$cluster

table(k2.df$cluster.4, droplevels(status[idx,]$most_general))

table(k2.df$cluster.4, k2.df$system)
table(k2$cluster, k2.df$system.spec)
table(k2$cluster, k2.df$organism)
table(k2$cluster, k2.df$septic)
...
p<-ggplot(k2.df[bct,], aes(k2.df$cluster.4[bct], fill=category)) +
  labs(title = "Barplot of Microbiology Results by Cluster", x = "Diagnosis", y = "Counts")+
  scale_fill_manual(values=c('#6c5ddd', '#dd5dbb'), 'Micro')+
  geom_bar()
p
...
### K7
set.seed(42)
k7 <- kmeans(X.t, centers = 7, nstart = 50)
...
p<-ggplot(status[idx,], aes(most_general, fill=k7$cluster)) +
  ...
  geom_bar()
p
# api_create(p, filename = "barplot_k7_dx")
...
plot_ly(pair1, type="scatter", x = ~PC1, y = ~PC2, mode = "markers", color = ~k7$cluster, size = status[idx,]$Age..months.,
        colors=k7.pal, text= ~paste0('age: ', status[idx,]$Age..months., '<br>WBC: ', status[idx,]$WBC, '<br>CRP: ',status[idx,]$array.contemporary.CRP, '<br>label:',status[idx,]$my_category_2, '<br>Diagnosis: ',status[idx,]$Diagnosis)) %>%
  layout(title = 'Cluster Assignment on PC 1-2')
...
p<-plot_ly(status[idx,], x = k7$cluster, y = ~status[idx,]$WBC, type = "box",
           color = ~k7$cluster, colors = c(k7.pal)) %>%
  layout(boxmode = "group",
         title = 'Box and Whisker Plot of WBC Count by Diagnostic Group, Split by Gender', 
         xaxis = list(title = 'Diagnosis'),
         yaxis = list(title = 'WBC Count'))
...
p<-plot_ly(status[idx,], x = k7$cluster, y = ~as.numeric(as.character(status[idx,]$array.contemporary.CRP)),
           type = "box", color = ~k7$cluster, colors = c(k7.pal)) %>%
  layout(boxmode = "group",
         title = 'Box and Whisker Plot of WBC Count by Diagnostic Group, Split by Gender', 
         xaxis = list(title = 'Diagnosis'),
         yaxis = list(title = 'CRP Count'))
...
# k7 analysis
k2.df$cluster.7 <- k7$cluster

table(k2.df$cluster.7, droplevels(status[idx,]$most_general))

table(k2.df$cluster.7, k2.df$system)
table(k2.df$cluster.7, k2.df$system.spec)
...
p<-ggplot(k2.df[bct,], aes(k2.df$cluster.7[bct], fill=category)) +
  labs(title = "Barplot of Microbiology Results by Cluster", x = "Diagnosis", y = "Counts")+
  scale_fill_manual(values=c('#6c5ddd', '#dd5dbb'), 'Micro')+
  geom_bar()
ggplotly(p)
...
table(droplevels(k7.df$category[k7.df$most_general=='bacterial']), droplevels(k7.df$cluster[k7.df$most_general=='bacterial']))
chisq.test(table(droplevels(k7.df$category[k7.df$most_general=='bacterial']), droplevels(k7.df$cluster[k7.df$most_general=='bacterial'])))
